...
import { ModulesList } from "../../Navigation/Menu/MenuList";

//Need some other way to store all urls
const navUrls = [
  { path: "/favorites", title: "Favorites" },
  { path: "/notifications-and-workflow", title: "Notifications and Workflow" },
  { path: "/project-management", title: "Project Management" },
  { path: "/capa", title: "CAPA" },
...
const TitleTab = ({ tabTitle, setTabTitle, children }) => {
  ...
  useEffect(() => {
    if (tabTitle === "") setIsTabOpen(false);
    else setIsTabOpen(true);
  }, [tabTitle]); /* ???? */
      ...
      <div className="t-a-b">
        <div className="tab-wrapper">
          <div className="tab-button active">
            ...
            <div className="text with-close">{tabTitle}</div>
